...
export default function CalendarPage() {
  ...
  function loadPlan(planId: string) {
    ...
    setWeekDays(weekdayLabels.map((weekday) => {
  ...
  function savePlan() {
    ...
    if (!name) {
      setPlanMessage("Doplň název šablony.");
      return;
    }
    if (!weekDays.some((day) => day.templateId)) {
      setPlanMessage("Vyber alespoň jeden trénink v týdnu.");
      return;
    }
  ...
  function generatePlan() {
    const activeDays = weekDays.filter((day) => day.templateId);
    ...
    >
      <section className="rounded-3xl border border-zinc-800 bg-zinc-900 p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label>
            ...
            <input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="mt-2 w-full rounded-2xl border border-zinc-700 bg-zinc-800 px-4 py-3.5" />
          </label>
          <label>
            <span className="text-sm font-bold text-zinc-300">Čas</span>
            <input type="time" value={time} onChange={(event) => setTime(event.target.value)} className="mt-2 w-full rounded-2xl border border-zinc-700 bg-zinc-800 px-4 py-3.5" />
          </label>
        </div>
        <button type="button" onClick={addToCalendar} className="mt-5 w-full rounded-2xl bg-lime-400 px-5 py-4 font-black text-zinc-950">
          Naplánovat trénink
        </button>
      ...
      <section className="mt-6 rounded-3xl border border-lime-400/20 bg-zinc-900 p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-lime-400">Rozvrh</p>
            <h2 className="mt-2 text-2xl font-black">Týdenní plán Po–Ne</h2>
            <p className="mt-2 text-sm text-zinc-400">Prázdný den znamená volno.</p>
          </div>
        ...
        {data.weeklyPlans.length > 0 && (
          <label className="mt-5 block">
            <span className="text-sm font-bold text-zinc-300">Načíst uloženou šablonu</span>
            <select defaultValue="" onChange={(event) => loadPlan(event.target.value)} className="mt-2 w-full rounded-2xl border border-zinc-700 bg-zinc-800 px-4 py-3.5">
              <option value="" disabled>Vyber šablonu</option>
              {data.weeklyPlans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name}</option>)}
            </select>
          </label>
        )}

        <label className="mt-5 block">
          <span className="text-sm font-bold text-zinc-300">Název šablony</span>
          <input value={planName} onChange={(event) => setPlanName(event.target.value)} className="mt-2 w-full rounded-2xl border border-zinc-700 bg-zinc-800 px-4 py-3.5" />
        </label>
          ...
          {weekdayLabels.map((weekday) => {
            ...
            return (
              <div key={weekday.value} className="grid grid-cols-[3.25rem_1fr] gap-3 rounded-2xl bg-zinc-800/70 p-3 sm:grid-cols-[6rem_1fr_8rem] sm:items-center">
                ...
                <select value={day.templateId ?? ""} onChange={(event) => updateWeekDay(weekday.value, { templateId: event.target.value || null })} className="min-w-0 rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-3 text-sm">
                  <option value="">Volno</option>
                  {data.templates.map((template) => <option key={template.id} value={template.id}>{template.title}</option>)}
                </select>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <label>
            <span className="text-sm font-bold text-zinc-300">Týden od</span>
            <input type="date" value={weekStart} onChange={(event) => setWeekStart(event.target.value)} className="mt-2 w-full rounded-2xl border border-zinc-700 bg-zinc-800 px-4 py-3.5" />
          </label>
          <div>
            <span className="text-sm font-bold text-zinc-300">Délka plánu</span>
            <div className="mt-2 grid grid-cols-3 gap-2">
              {[4, 8, 12].map((value) => (
                <button key={value} type="button" onClick={() => setWeeks(value as 4 | 8 | 12)} className={`rounded-2xl px-3 py-3.5 font-black ${weeks === value ? "bg-lime-400 text-zinc-950" : "bg-zinc-800 text-zinc-300"}`}>
                  {value} týdnů
                </button>
              ))}
            </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <button type="button" onClick={savePlan} className="rounded-2xl border border-zinc-700 px-5 py-4 font-bold text-zinc-200">Uložit šablonu</button>
          <button type="button" onClick={generatePlan} className="rounded-2xl bg-lime-400 px-5 py-4 font-black text-zinc-950">Vygenerovat plán</button>
        </div>

        {data.weeklyPlans.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2">
            {data.weeklyPlans.map((plan) => (
              <button key={plan.id} type="button" onClick={() => deleteWeeklyPlan(plan.id)} className="rounded-full bg-zinc-800 px-3 py-2 text-xs font-bold text-zinc-400">
                Smazat: {plan.name}
              </button>
            ))}
          </div>
        ...
        {items.map((item) => {
          ...
          return (
            <article key={item.id} className="rounded-3xl border border-zinc-800 bg-zinc-900 p-5">
              <div className="flex items-start justify-between gap-4">
                <div><p className="capitalize text-sm font-bold text-lime-400">{dateLabel(item.date)}</p><h3 className="mt-1 text-2xl font-black">{template.title}</h3><p className="mt-1 text-sm text-zinc-500">{template.durationMinutes} min</p></div>
                <StatusBadge status={item.status} />
              </div>
              <div className="mt-5 grid grid-cols-2 gap-3">
                <label><span className="text-xs font-bold uppercase tracking-wide text-zinc-500">Přesunout na</span><input type="date" value={item.date} onChange={(event) => updateScheduledWorkout(item.id, { date: event.target.value })} className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-800 px-3 py-3 text-sm" /></label>
                <label><span className="text-xs font-bold uppercase tracking-wide text-zinc-500">Čas</span><input type="time" value={item.time} onChange={(event) => updateScheduledWorkout(item.id, { time: event.target.value })} className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-800 px-3 py-3 text-sm" /></label>
              </div>
