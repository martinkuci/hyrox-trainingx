      ...
      <section className="ui-card p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <label className="sm:col-span-2">
            ...
            <select value={templateId || data.templates[0]?.id || ""} onChange={(event) => setTemplateId(event.target.value)} className="ui-field mt-2">
          ...
          <label><span className="text-sm font-bold text-zinc-300">Datum</span><input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="ui-field mt-2" /></label>
          <label><span className="text-sm font-bold text-zinc-300">Čas</span><input type="time" value={time} onChange={(event) => setTime(event.target.value)} className="ui-field mt-2" /></label>
        </div>
        <button type="button" onClick={addToCalendar} className="ui-button ui-button-primary mt-5 w-full">Naplánovat trénink</button>
        {message && <p role="status" className="ui-feedback ui-feedback-success mt-4 text-center text-sm font-semibold">{message}</p>}
      </section>

      <section className="ui-card ui-card-accent mt-6 p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div><p className="text-xs font-black uppercase tracking-[0.2em] text-accent">Rozvrh</p><h2 className="mt-2 text-2xl font-black">Týdenní plán Po–Ne</h2><p className="mt-2 text-sm text-zinc-400">Prázdný den znamená volno.</p></div>
          <span className="ui-chip ui-chip-accent shrink-0">4 / 8 / 12 týdnů</span>
        </div>

        {data.weeklyPlans.length > 0 && (
          <label className="mt-5 block"><span className="text-sm font-bold text-zinc-300">Načíst uloženou šablonu</span><select defaultValue="" onChange={(event) => loadPlan(event.target.value)} className="ui-field mt-2"><option value="" disabled>Vyber šablonu</option>{data.weeklyPlans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name}</option>)}</select></label>
        )}

        <label className="mt-5 block"><span className="text-sm font-bold text-zinc-300">Název šablony</span><input value={planName} onChange={(event) => setPlanName(event.target.value)} className="ui-field mt-2" /></label>

        <div className="mt-5 space-y-3">
          {weekdayLabels.map((weekday) => {
            const day = weekDays.find((item) => item.weekday === weekday.value)!;
            return (
              <div key={weekday.value} className="ui-inset grid grid-cols-[3.25rem_1fr] gap-3 p-3 sm:grid-cols-[6rem_1fr_8rem] sm:items-center">
                <div className="font-black text-accent"><span className="sm:hidden">{weekday.short}</span><span className="hidden sm:inline">{weekday.label}</span></div>
                <select value={day.templateId ?? ""} onChange={(event) => updateWeekDay(weekday.value, { templateId: event.target.value || null })} className="ui-field min-w-0 bg-surface px-3 py-3 text-sm"><option value="">Volno</option>{data.templates.map((template) => <option key={template.id} value={template.id}>{template.title}</option>)}</select>
                <input type="time" value={day.time} disabled={!day.templateId} onChange={(event) => updateWeekDay(weekday.value, { time: event.target.value })} className="ui-field col-start-2 bg-surface px-3 py-3 text-sm sm:col-start-auto" />
              </div>
            );
          })}
        </div>

        <div className="ui-inset mt-4 flex items-center justify-between p-4 text-sm">
          <span className="text-zinc-400">Vybrané dny: <strong className="text-white">{selectedDayNames || "žádné"}</strong></span>
          <span className="font-black text-accent">{plannedCount} tréninků</span>
        </div>

        <button type="button" onClick={clearWeek} className="ui-button ui-button-ghost ui-button-sm mt-3 w-full">Vyčistit týden</button>

        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <label><span className="text-sm font-bold text-zinc-300">Týden od</span><input type="date" value={weekStart} onChange={(event) => setWeekStart(event.target.value)} className="ui-field mt-2" /></label>
          <div><span className="text-sm font-bold text-zinc-300">Délka plánu</span><div className="mt-2 grid grid-cols-3 gap-2">{[4, 8, 12].map((value) => <button key={value} type="button" aria-pressed={weeks === value} onClick={() => setWeeks(value as 4 | 8 | 12)} className="ui-choice px-2 py-3.5">{value} týdnů</button>)}</div></div>
        </div>

        <div className="mt-5 grid gap-3 sm:grid-cols-2"><button type="button" onClick={savePlan} className="ui-button ui-button-outline">Uložit šablonu</button><button type="button" onClick={generatePlan} className="ui-button ui-button-accent">Vygenerovat plán</button></div>
        ...
        {pendingGenerated && (
          <div className="ui-feedback ui-feedback-warning mt-5">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-3">
              <button type="button" onClick={() => resolveConflicts("skip")} className="ui-button ui-button-secondary ui-button-sm">Přeskočit</button>
              <button type="button" onClick={() => resolveConflicts("replace")} className="ui-button ui-button-primary ui-button-sm">Nahradit</button>
              <button type="button" onClick={() => resolveConflicts("duplicate")} className="ui-button ui-button-outline ui-button-sm">Duplikovat</button>
            </div>
          </div>
        )}

        {data.weeklyPlans.length > 0 && <div className="mt-4 flex flex-wrap gap-2">{data.weeklyPlans.map((plan) => <button key={plan.id} type="button" onClick={() => deleteWeeklyPlan(plan.id)} className="ui-button ui-button-danger ui-button-sm rounded-full text-xs">Smazat: {plan.name}</button>)}</div>}
