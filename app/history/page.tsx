  ...
  return (
    <main className="min-h-dvh bg-zinc-950 px-5 py-8 text-white">
      <div className="mx-auto max-w-lg">
        ...
        <p className="mt-9 text-sm font-black uppercase tracking-[0.22em] text-lime-400">
          HYROX Training
        </p>
            ...
            return (
              ...
              >
                <div className="p-6">
                  ...
                  {result.weights && (
                    <div className="mt-4">
                      <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Váhy</p>
                      <p className="mt-1 text-zinc-200">{result.weights}</p>
                    </div>
                  )}
                  {result.notes && (
                    <div className="mt-4">
                      <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Poznámka</p>
                      <p className="mt-1 whitespace-pre-wrap text-zinc-300">{result.notes}</p>
                    </div>
                  )}
                  ...
                  <div className="mt-5 grid grid-cols-[1fr_auto] gap-3">
                    <button
                      ...
                      className="rounded-xl bg-zinc-800 px-4 py-3 text-sm font-semibold text-zinc-200"
                    >
                      {expanded ? "Skrýt mezičasy" : "Zobrazit mezičasy"}
                    </button>
                    <button
                      type="button"
                      aria-label="Smazat výsledek"
                      onClick={() => setPendingDelete(result)}
                      className="rounded-xl border border-red-500/30 px-4 py-3 text-sm font-semibold text-red-300"
                    >
                      Smazat
                    </button>
                  </div>
      ...
      <ConfirmDialog
        ...
        description="Tento záznam už nepůjde obnovit."
        confirmLabel="Smazat"
        destructive
        onCancel={() => setPendingDelete(null)}
        onConfirm={() => {
          if (pendingDelete) deleteResult(pendingDelete.id);
