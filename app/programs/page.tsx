  ...
  return (
    <PlanningShell eyebrow="Plán" title="Nový tréninkový program" description="Vyber cíl, délku, frekvenci a dostupné dny. Aplikace sestaví konkrétní tréninky automaticky." backHref="/plan">
      ...
      {weeks.length > 0 && <>
        ...
        <div className="mt-4 space-y-4">{weeks.map((week, weekIndex) => <section key={week.weekNumber} className="ui-card p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-accent">Týden {week.weekNumber}</p><h3 className="mt-1 text-lg font-black">{week.focus}</h3></div><span className="ui-chip">{phaseLabels[week.phase]}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{week.sessions.map((session, sessionIndex) => <label key={session.id} className="ui-inset p-3"><span className="text-xs font-black uppercase tracking-wide text-accent">Jednotka {sessionIndex + 1}</span><select value={session.templateId ?? ""} onChange={(e) => updateSession(weekIndex, sessionIndex, e.target.value)} className="ui-field mt-2 bg-surface px-3 py-3 text-base">{data.templates.map((template) => <option key={template.id} value={template.id}>{template.metadata?.workoutCode ? `${template.metadata.workoutCode} · ` : ""}{template.title}</option>)}</select></label>)}</div></section>)}</div>
