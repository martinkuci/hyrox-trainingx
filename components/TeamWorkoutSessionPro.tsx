  ...
  return (
    <main className="runner-shell safe-screen h-dvh overflow-hidden px-3 text-white"><section className="mx-auto flex h-full w-full max-w-md flex-col gap-2 py-2">
      ...
      <section className="min-h-0 flex-1 rounded-[1.5rem] border border-white/10 bg-white/[0.025] p-3 shadow-inner shadow-black/20"><div className="flex h-full flex-col justify-center">
        ...
        </> : <div className="text-center"><p className="text-lg font-black">{meCompletedCurrent ? "✓ HOTOVO · ČEKÁM NA PARTNERA" : "Připrav se"}</p><p className="mt-1 text-xs text-zinc-500">{meCompletedCurrent ? "Partner dokončuje svůj úsek." : `Teď pracuje ${activeName ?? "tvůj parťák"}. Stav se přepne automaticky.`}</p></div>}
