...
export default function WorkoutResultForm({ template, scheduledWorkoutId, durationSeconds, splits }: Props) {
  ...
  if (saved) return <main className="safe-screen flex min-h-dvh items-center justify-center bg-zinc-950 px-5 text-white"><section className="w-full max-w-md rounded-3xl border border-zinc-800 bg-zinc-900 p-7 text-center"><div className="mx-auto grid size-16 place-items-center rounded-full bg-lime-400 text-3xl font-black text-zinc-950">✓</div><p className="mt-6 text-sm font-bold uppercase tracking-[0.2em] text-lime-400">Hotovo</p><h1 className="mt-2 text-3xl font-bold">Výsledek je uložený</h1><p className="mt-3 text-zinc-400">Uložili jsme i kód, verzi a cílový profil tréninku pro budoucí porovnání.</p><div className="mt-8 grid gap-3"><Link href="/history" className="rounded-2xl bg-lime-400 px-5 py-4 font-bold text-zinc-950">Zobrazit historii</Link><Link href="/" className="rounded-2xl border border-zinc-700 px-5 py-4 font-semibold">Zpět na přehled</Link></div></section></main>;

  return <main className="safe-screen min-h-dvh bg-zinc-950 px-5 text-white"><section className="mx-auto max-w-md"><p className="text-sm font-bold uppercase tracking-[0.22em] text-lime-400">Trénink dokončen</p>{template.metadata?.workoutCode && <p className="mt-3 inline-block rounded-full bg-lime-400 px-3 py-1 text-xs font-black text-zinc-950">{template.metadata.workoutCode}-V{template.metadata.templateVersion}</p>}<h1 className="mt-2 text-3xl font-bold">{template.title}</h1>
