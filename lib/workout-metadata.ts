...
export function normalizeMetadata(metadata?: Partial<WorkoutMetadata>): WorkoutMetadata | undefined {
  ...
  if (!workoutCode && !goal && !primaryMetric && !progressionGroup && !runningTarget && secondaryMetrics.length === 0) {
    return undefined;
  }
  ...
  return {
    ...
    category: metadata.category ?? base.category,
    goal,
    targetRpeMin: minRpe,
    targetRpeMax: maxRpe,
